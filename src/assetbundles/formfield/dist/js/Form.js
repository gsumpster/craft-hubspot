...
(function ($, window, document, undefined) {
  ...
  Plugin.prototype = {
    ...
    updatePreview: function (value) {
      console.debug(
        "HubSpot for CraftCMS: Input Preview Updating",
      );
      var _this = this;

      _this.element.querySelector(`#fields-${_this.options.name}`).onchange =
      () =>
        _this.updatePreview(
          _this.element.querySelector(`#fields-${_this.options.name}`)
            .selectize.items[0]
        );
      ...
      if (value) {
        ...
        const detailsCode = `
                        <div class="hsff-preview-details">
                            <p class="hsff-preview-details__title">${
                              form.isPublished &&
                              `<span class="hsff-preview-details__status"></span>`
                            }${form.name}</p>

                            <div>
                                <a class="hsff-preview-details__edit" target="_blank" href="https://app.hubspot.com/forms/${
                                  form.portalId
                                }/editor/${
          form.guid
        }/edit/form">Edit in Hubspot</a>
                                ${
                                  !_this.options.disablePreview &&
        ...
        if (!_this.options.disablePreview) {
          ...
          _this.element.querySelector(
            `#${_this.options.name}-preview`
          ).onclick = () =>
            _this.element
              .querySelector(".hsff-preview")
              .classList.toggle("hsff-preview--open");
        }
      }
    },
  };
